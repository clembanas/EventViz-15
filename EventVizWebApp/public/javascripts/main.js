$(document).ready(function(){
	...
	*/
	...
	function clickMarker(marker){
		...
		currentEvent.id = options.eventfulID;
		$.getJSON( "/getEventById", { id : currentEvent.id }).done(function(result){
			console.log(result);
			currentEvent = result;
			currentCity = null;
			currentCountry = null;
			infoboxEvent();
			var terms = "['data1', 'data2', 'data3']";
			$.getJSON( "/getSentiment", { terms: terms, location: currentEvent.city } ).done(function(result){
	...
	function infoboxEvent(){
		$("#info").empty();
		$("#info").append($("<p id=eventName>").text("Name: " + currentEvent.event.name));
		$("#info").append($("<p id=eventDescription>").text("Description: " + (currentEvent.event.description == "" ? "Not specified" : currentEvent.event.description)));
		...
		if(bands.length != 0){
			...
			for(var i = 0; i < bands.length; i++){
				var idName = "band" + bands[i].name.replace(/[^a-z0-9]/gmi, "").replace(/\s+/g, "");;
